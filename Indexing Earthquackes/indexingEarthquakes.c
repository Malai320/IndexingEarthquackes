/*
 Format of txt file attahced to this project:
 (country, earthquacke magnitude, latitude, longitude, tsunami)
 for tsunami: 1 - there was a tsunami, 2 - there was no tsunami

 This is a program that takes a txt file (there is a sample file inside the project)
 and stores the data from that file into binary tree data structure according to english
 alphabet. This data structure stores small elements on the left of the tree and big
 elements on the right. Because ASCII values of characters were used, if the root node
 of the tree begins with "C" character, then all the nodes on the left branch of the tree
 will begin with A and B, because their ASCII values are less than the ASCII value of "C".
 Subsequently, all the letters that go after C in the english alphabet will go to the
 right branch of the root node. This is done so that if we use in-order traversal of the
 tree, we will get all the countries in alphabetical order.

 After storing them in a tree, the program gives you several options.

 Option 1 simply prints all the countries in alphabetical order.

 Option 2 counts how many earthquackes there are in each of the countries
 in our list (there can be more than 1 earthquacke in some country) and
 displays the country/countries with the biggest number of earthquackes.

 Option 3 displays the country with the earthquacke of the weakest magnitude.

 Time complexities of certain functions are also discussed in this code.
